...
sudo apt install build-essential alsa-utils libasound2-dev libssl-dev \
               libjson-c-dev opus-tools libopus-dev libopusenc-dev libopusfile-dev  \
               cmake pkg-config

# 如果遇到 "E: Unable to locate package libopusenc-dev"，请先运行：
...
### 1. 环境设置（如果从源码安装了库）
如果从源码安装了libopusenc等库，可能需要设置环境变量：
```bash
# 设置PKG_CONFIG_PATH以确保构建脚本能找到库
...
### 2. 修复脚本换行符问题（如果遇到`#!/bin/bash: No such file or directory`错误）
```bash
# 修复Windows创建的脚本在Linux上的换行符问题
sed -i 's/\r$//' scripts/*.sh
```

### 2.1 修复pkg-config文件缺失问题（当库文件存在但pkg-config找不到时）
```bash
# 检查是否有.pc文件
